...
import { fabricSwatches, colorFilters, type SwatchFabric } from '@tara-hub/lib/fabric-data'

// Swatch Box Context (simplified for now - would use SwatchContext in production)
interface SwatchBoxItem {
  id: string
  name: string
  image: string
  sku: string
            ...
            {categories.map((category) => (
              <label key={category} className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-500">
                  ({fabricSwatches.filter(f => f.category === category).length})
                </span>
              </label>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {patterns.map((pattern) => (
              <label key={pattern} className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-gray-500">
                  ({fabricSwatches.filter(f => f.pattern === pattern).length})
                </span>
              </label>
            ))}
          </div>
    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group">
      <div className="aspect-square relative overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex gap-2 flex-wrap">
          {fabric.properties.slice(0, 2).map((prop, idx) => (
...
export default function BrowsePage() {
  ...
  const filteredFabrics = fabricSwatches.filter(fabric => {
    // Search filter
    if (searchTerm) {
